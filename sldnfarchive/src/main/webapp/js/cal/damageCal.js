var apiKey = "e5bpFIpV8gTY3BTZn0NQ9a8Tdj0rVls7";
var flag = "";
var maxLcd = "";
var maxScd = [];
...
function schCal() {
	var data = $("#schCalFrm").serializeObject();
	var path = "/cal/calList.do";
	var js = "/js/cal/damageCal.js";
	
	$(".card-body").children().remove();
	$(".card-body").load(path + " #calList", data, function() {
		$.getScript(js);
	});
}
...
function selectCal(e) {
	var data = {};
	var serverId = $(e).siblings("div[id^='serv_']").attr("id").split("serv_")[1];
	var characterId = $(e).closest("div[id^='info']").attr("id").split("info")[1];
	var path = "/cal/selectCal.do";
	var js = "/js/cal/calDetail.js";
	
	data.serverId = serverId;
	data.characterId = characterId;
	
	$(".card-body").children().remove();
	$(".card-body").load(path + " #calDetail", data, function() {
		$.getScript(js);
	});
}
